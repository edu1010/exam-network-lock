...
/// <summary>
/// Broadcasts this student's shield state and tamper-evident log to the teacher's ExamMonitor over
/// the LAN (UDP, send-only) using the shared <see cref="MonitorBroadcaster"/>. Because the wire
/// protocol lives in ExamShared, the Linux client is fully interoperable with the Windows monitor.
/// While the network is down the broadcasts no-op; once the student restores the connection they
